                        ...
                        std::vector<size_t> &cells)
{
    ...
    gi = size_t(std::max((float)0.,(float)floor(x*n_cells_x)));
    gj = size_t(std::max((float)0.,(float)floor(y*n_cells_y)));
    gk = size_t(std::max((float)0.,(float)floor(z*n_cells_z)));
...
void Uniform_Grid::query_cell(float x, float y, float z, size_t &i, size_t &j, size_t &k)
{
        ...
        //Move from [0,1] -> "grid space"
        i = size_t(std::max((float)0.,(float)floor(x*n_cells_x)));
        j = size_t(std::max((float)0.,(float)floor(y*n_cells_y)));
        k = size_t(std::max((float)0.,(float)floor(z*n_cells_z)));
